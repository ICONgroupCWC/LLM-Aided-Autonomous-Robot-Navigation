...
def draw_boxes_cv2_one(img, boxes, confs, clss,cls_dict): 
    ...
    class_name = cls_dict[int(clss)]
    x1 = int(boxes[0])
    y1 = int(boxes[1])
    x2 = int(boxes[2])
    y2 = int(boxes[3])
    img = cv2.rectangle(img, (x1, y1), (x2, y2), colors[int(clss)], 2)
    ...
    string1 = class_name + "  " + str(round(confs, 2))
    img = cv2.rectangle(img, (rx1, ry1), (rx2, ry2), colors[int(clss)], -1)
    img = cv2.putText(img, string1, (prx1, pry1), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (255,255,255), 1)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if(goal_flag==False):
            ...
            if(len(confs)>0 and obj_id != 'None'):
                print('detected')
                check_flag = True
                for i in range(len(clss)):
                    ...
                    if(cls_dict[clss[i]]==obj_id):
                        ...
                        image_src = draw_boxes_cv2_one(image_src, boxes[i], confs[i], clss[i],cls_dict)
                        box = boxes[i]
                        conf_ = confs[i]
                        cls_ = clss[i]
                ...
                if(check_flag==True):
                    detect_flag = 0
                    print("can not find the "+str(obj_id))
                    if(loop_indx<330):
                        move_cmd.linear.x = 0.0
                        move_cmd.linear.y = 0.0
                        move_cmd.linear.z = 0.0
