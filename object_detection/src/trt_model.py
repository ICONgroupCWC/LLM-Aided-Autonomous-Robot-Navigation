...
def allocate_buffers(engine):
    ...
    #cleanup_HostDeviceMem(host_mem, device_mem)
...
def postprocess_yolo(trt_outputs, img_w, img_h, conf_th, nms_threshold,
    ...
    if len(detections) == 0:
        ...
        for class_id in set(detections[:, 5]):
            ...
            keep = nms_boxes(cls_detections, nms_threshold)
...
def detect(engine, img,input_shape,inputs, outputs, bindings,context, stream, conf_th=0.3,):
    ...
    img_resized = preprocess_yolo(img, input_shape, letter_box)
    ...
    boxes, scores, classes = postprocess_yolo(trt_outputs, img.shape[1], img.shape[0], conf_th, nms_threshold=0.5, input_shape=input_shape,letter_box=letter_box)
